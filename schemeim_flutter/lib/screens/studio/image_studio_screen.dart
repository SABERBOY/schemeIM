  ...
  Future<void> _generateImage() async {
    ...
    if (apiKey.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("API Key missing. Use --dart-define=API_KEY=..."),
        ),
      );
      return;
    }
    ...
    try {
      final model = GenerativeModel(
        ...
        apiKey: apiKey,
      ); // Using 1.5 Flash as standard available model
      ...
      if (!foundImage) {
        if (response.text != null) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                "Model returned text: ${response.text!.substring(0, min(50, response.text!.length))}...",
              ),
            ),
          );
        } else {
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(const SnackBar(content: Text("No image generated.")));
        }
      }
    } catch (e) {
